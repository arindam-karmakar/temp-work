...
class DriveTime extends StatefulWidget {
  DriveTime({Key? key}) : super(key: key);

  @override
  _DriveTimeState createState() => _DriveTimeState();
}

class _DriveTimeState extends State<DriveTime> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<List<CdlSlot>> getStudents() => WebApi().getSlotByDate(
        cdlSchool.schoolId.toString(),
        DateFormat('yyyy-MM-dd', 'en_US').format(initialDateTime));
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return GestureDetector(
                            ...
                            child: (lastStudent == index + 1)
                                ? PressedCardWithText(
                                    name: snapshot.data[index].enroll.name,
                                    phone: snapshot.data[index].enroll.phone,
                                    image: snapshot.data[index].enroll.image,
                                    inText: Text(
                                      DateFormat('HH:mm', 'en_US').format(
                                              snapshot.data[index].from) +
                                          " - " +
                                          DateFormat('HH:mm', 'en_US')
                                              .format(snapshot.data[index].to),
                                      style: TextStyle(
                                        color: kPending,
